...
//下面两个函数处理各处别名，统一改成_alias_t_x格式
int mysql_format_table_alias(THD* thd)
{
    
    TABLE_LIST* tables;
    TABLE_LIST* table;
    TABLE_LIST* table_global;
    ...
    if ((thd->lex->sql_command == SQLCOM_INSERT_SELECT ||
         ...
         thd->lex->sql_command == SQLCOM_UPDATE_MULTI ||
         thd->lex->sql_command == SQLCOM_UPDATE))
        tables = thd->lex->select_lex.table_list.first->next_local;
    else
    ...
    {
        int num= 1;
        for (table= tables, table_global=tables; table; table= table->next_local)
        {
            
            if (table->is_view_or_derived())
            {
                sprintf(table->alias, "_alias_t_%d", num++);
                sprintf(table_global->alias, "%s", table->alias);
            }
            if(table_global)
            {
                table_global= table->next_global;
            }
        }
    }
...
int mysql_format_filed_table_name(
                                  THD* thd,
                                  Item_field* item
                                  )
{
    ...
    if(tables)
    {
        for (table_global= tables; table_global; table_global= table_global->next_global)
        {
            if(table_global->table_name && item->table_name && strcmp(table_global->table_name, item->table_name)==0)
            {
                item->table_name= table_global->alias;
            }
        }
    }
    return 0;
                        ...
                        )
{
    ...
    if (tables)
    {
        ...
        for (table= tables; table; table= table->next_local)
        {
            ...
            if (table->is_view_or_derived())
            {
                mysql_format_subselect(thd, format_node, print_str, table->derived->first_select(), false);
                sprintf(tablename, " %s", table->alias);
                str_append(print_str, tablename);
            }
            else
            ...
            )
{
    ...
    switch (item->type()) {
        ...
        case Item::FIELD_ITEM:
        {
            ...
            mysql_format_filed_table_name(thd, (Item_field*)item);
            ...
            if (((Item_field*)item)->table_name)
                sprintf(tablename, "%s.", ((Item_field*)item)->table_name);
            tablert = mysql_find_field_from_all_tables(
                                                       thd, &format_node->rt_lst, select_lex, ((Item_field*)item)->db_name,
                                                       ((Item_field*)item)->table_name, ((Item_field*)item)->field_name);
            if (tablert)
            {
                if (strcasecmp(((Item_field*)item)->field_name, "*"))
