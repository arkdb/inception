...
void print_select_sql(THD *thd)
{
    ...
    sql= &sql_space;
    sql= str_init(sql);
    sql= str_append(sql, "select ");
    
    int sig = 0;
    while (!thd->lex->select_lex.item_list.is_empty())
    {
        Item* item = thd->lex->select_lex.item_list.pop();
        if (sig==1)
            sql= str_append(sql,", ");
        sql= str_append(sql,item->full_name());
        sig==1?:sig=1;
    }
    sql= str_append(sql, " from ");
    sql= str_append(sql, thd->lex->select_lex.table_list.first->db);
    sql= str_append(sql, ".");
    sql= str_append(sql, thd->lex->select_lex.table_list.first->table_name);
    sql= str_append(sql, " where ");
    
    Item_func* item_fc = (Item_func*)thd->lex->select_lex.where;
    if (item_fc->arg_count == 2)
    {
        sql= str_append(sql, item_fc->arguments()[0]->full_name());
        sql= str_append(sql, "=");
        sql= str_append(sql, item_fc->arguments()[1]->full_name());
    }
    
/*    if (thd->lex->select_lex.where->type() == Item_basic_constant::FUNC_ITEM)
    {
        Item_func* item_fc = (Item_func*)thd->lex->select_lex.where;
        if(item_fc->functype() == Item_bool_func::EQ_FUNC)
        {
            item_fc->
            Item_func_eq* item_fc_eq = (Item_func_eq*)item_fc;
            sql= str_append(sql, Item_func_eq);
        }
    }  */
    
    sprintf(select, "%s", sql);
    sql_print_information((char *)select);
}

void inception_spell_sql(THD *thd)
{
    switch (thd->lex->sql_command)
    {
        case SQLCOM_CHANGE_DB:
